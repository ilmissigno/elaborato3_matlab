...
if (MAXITER==numiter)
    ...
    warning('Il numero di iterazioni effettuate non è sufficiente per raggiungere l''accuratezza desiderata. niter=%d, residuo_relativo=%s',numiter,resrel);
end
...
if ((~isnumeric(A)) || (any(find(isinf(A)))) || (any(find(isnan(A)))) || (~isreal(A)) || (isempty(A)))
    error('Uno o più valori inseriti in A non sono validi');
end
...
if ((~isnumeric(b)) || (any(find(isinf(b)))) || (any(find(isnan(b)))) || (~isreal(b)) || (isempty(b)))
    error('Uno o più valori inseriti nel vettore non sono validi');
end
end
...
if (TOL < eps)
    warning('Il valore di TOL specificato è troppo piccolo. Verrà usato il valore di default TOL = 10^-6');
    TOL = 10^-6;
end

if (TOL >= 1)
    warning('Il valore di TOL specificato è troppo grande. Il risultato fornito potrebbe essere molto inaccurato. Si guardi la documentazione.');
end
end
...
if (MAXITER < 10)
    warning('Il numero di iterazioni specificato è molto piccolo, l''errore di calcolo potrebbe essere elevato');
end
