...
% Last Modified by GUIDE v2.5 26-Apr-2019 13:05:22
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
dimensioneA = get(handles.dimA,'String');
dimensioneB = get(handles.dimB,'String');
toll = get(handles.tol,'String');
nummax = get(handles.nmax,'String');
if(isempty(dimensioneA))
    errordlg('Inserire una dimensione per la matrice');
    error('Inserire una dimensione per la matrice');
elseif(isempty(dimensioneB))
    errordlg('Inserire una dimensione per il vettore');
    error('Inserire una dimensione per il vettore');
else
    dimensioneA = str2double(dimensioneA);
    dimensioneB = str2double(dimensioneB);
...
if ((~isscalar(toll)) || (~isnumeric(toll)) || (isinf(toll)) || (isnan(toll)) || (toll >= 0))
    errordlg('L''esponente del valore di Tolleranza deve essere negativo','Errore');
